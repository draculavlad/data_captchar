...
def crack_captcha(captcha_image, captcha_label):
    ...
    with tf.Session(config=dz.config) as sess:
        ...
        for i in range(len(captcha_label)):
            ...
            predict = tf.argmax(tf.reshape(output, [-1, dz.max_captcha, dz.charset_len]), 2)
            text_list = sess.run(predict, feed_dict={dz.X: [img], dz.keep_prob: 1})
            text = text_list[0].tolist()
            vector = np.zeros(dz.max_captcha*dz.charset_len)
            i = 0
            for n in text:
                    vector[i*dz.charset_len + n] = 1
                    i += 1
...
if __name__ == '__main__':
    dz = train.Trainer()
    dz.get_imgs(dz.train_data_path, dz.test_data_path)
    batch_x, batch_y = dz.get_next_batch(False, 100)
